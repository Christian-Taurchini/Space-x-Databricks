...
## 8️⃣ Esempi Codice PySpark

**Ingestione JSON (Raw → Bronze):**

```python
from pyspark.sql import SparkSession
from pyspark.sql.functions import current_timestamp

spark = SparkSession.builder.appName("SpaceX Bronze Ingestion").getOrCreate()

df = spark.read.json("/data_lake/spacex/raw/launches/2026-04-08/launches.json")
df = df.withColumn("ingestion_timestamp", current_timestamp())
df.write.format("delta").mode("overwrite").save("/data_lake/spacex/bronze/launches")
```
...
silver_df = df.select(
    ...
    col("rocket").alias("rocket_id"),
    col("launchpad").alias("launchpad_id"),
    col("date_utc").alias("launch_date"),
    col("success")
).dropDuplicates()
```
...
```python
from pyspark.sql.functions import year, count, sum, col

gold_df = silver_df.groupBy(year("launch_date").alias("year")) \
    .agg(
        count("*").alias("total_launches"),
        sum(col("success").cast("int")).alias("successful_launches")
    ) \
    .withColumn("success_rate", col("successful_launches")/col("total_launches"))
```
